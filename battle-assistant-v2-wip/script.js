...
const teamDialogManager = document.getElementById('team-dialog-manager');
const teamDialogConflict = document.getElementById('team-dialog-conflict');
const teamDialogConflictSummary = document.getElementById('team-dialog-conflict-summary');
const teamDialogConflictMergeButton = document.getElementById('team-dialog-conflict-merge');
const teamDialogConflictOverwriteButton = document.getElementById('team-dialog-conflict-overwrite');
const teamDialogConflictSaveAsButton = document.getElementById('team-dialog-conflict-save-as');
const teamDialogConflictCancelButton = document.getElementById('team-dialog-conflict-cancel');
const teamDialogAction = document.getElementById('team-dialog-action');
const teamDialogActionTitle = document.getElementById('team-dialog-action-title');
const teamDialogActionSummary = document.getElementById('team-dialog-action-summary');
const teamDialogActionConfirmButton = document.getElementById('team-dialog-action-confirm');
const teamDialogActionCancelButton = document.getElementById('team-dialog-action-cancel');
...
const teamDialogSaveButton = document.getElementById('team-dialog-save');
const teamDialogLoadButton = document.getElementById('team-dialog-load');
...
let teamDialogConflictRecord = null;
let teamDialogActionState = null;
...
function bindEvents() {
    ...
    teamDialogSaveButton.addEventListener('click', function () {
        void confirmSaveTeam();
    });
    teamDialogLoadButton.addEventListener('click', function () {
    ...
    teamDialogConflictMergeButton.addEventListener('click', function () {
        void confirmConflictSave('merge');
    });

    teamDialogConflictOverwriteButton.addEventListener('click', function () {
        void confirmConflictSave('overwrite');
    });

    teamDialogConflictSaveAsButton.addEventListener('click', function () {
        showSaveAsState();
    });

    teamDialogConflictCancelButton.addEventListener('click', function () {
        hideConflictState();
        teamDialogNicknameInput.focus();
        teamDialogNicknameInput.select();
    });

    teamDialogActionConfirmButton.addEventListener('click', function () {
        void confirmInlineAction();
    });

    teamDialogActionCancelButton.addEventListener('click', function () {
        hideActionState();
        focusTeamDialogList();
    });
...
async function saveTeamRecord(record, allowReplace = false) {
    const db = await getTeamStorageDb();
    return new Promise((resolve, reject) => {
        const tx = db.transaction(teamStorageStoreName, 'readwrite');
        const store = tx.objectStore(teamStorageStoreName);
        const request = allowReplace ? store.put(record) : store.add(record);
...
async function openTeamManager(team) {
    if (!hasAnyPokemonInTeam(team)) {
        return;
    }

    try {
        await openTeamDialog(team);
    } catch (error) {
        console.error('Unable to open team manager', error);
        closeTeamDialog();
        alert('Unable to manage saved teams in this browser.');
    }
}
...
async function openTeamDialog(team) {
    teamDialogMode = 'manage';
    teamDialogTeam = team;
    teamDialogRecords = [];
    teamDialogConflictRecord = null;
    teamDialogActionState = null;
    teamDialog.hidden = false;

    teamDialogTitle.textContent = `Manage ${getTeamLabelForSide(teamDialogTeam)}`;
    teamDialogSubtitle.textContent = '';
    teamDialogSubtitle.hidden = true;
    teamDialogManager.hidden = false;
    teamDialogPreview.textContent = getTeamRosterPreviewText(teamDialogTeam);
    teamDialogNicknameInput.value = '';
    hideConflictState();
    hideActionState();
    teamDialogSaveButton.disabled = false;
    teamDialogLoadButton.disabled = false;
    teamDialogRemoveButton.disabled = false;

    await refreshTeamDialogRecords();
    teamDialogNicknameInput.focus();
    teamDialogNicknameInput.select();
}
...
async function refreshTeamDialogRecords() {
    const teamRecords = await getSavedTeamRecordsForSelectedTeam();
    teamDialogRecords = teamRecords;
    renderTeamDialogLoadOptions(teamRecords);
}

function renderTeamDialogLoadOptions(teamRecords) {
    teamDialogLoadList.innerHTML = '';
    ...
    if (teamRecords.length === 0) {
        ...
        teamDialogLoadButton.disabled = true;
...
function combineUniqueRosters(primaryNames, secondaryNames) {
    const seen = new Set();
    const combined = [];

    [...primaryNames, ...secondaryNames].forEach((name) => {
...
async function confirmSaveTeam() {
    try {
        ...
        const currentRoster = getTeamPokemonNames(teamDialogTeam);
        const existing = await getSavedTeamRecord(nickname);
        if (existing) {
            showConflictState(existing);
            return;
        }
...
function showConflictState(existing) {
    teamDialogConflictRecord = existing;
    if (teamDialogConflictSummary) {
        teamDialogConflictSummary.textContent = `Nickname "${existing.nickname}" already exists. You can merge the current team into it, overwrite it, save under a new name, or cancel.`;
    }
    if (teamDialogConflict) {
        teamDialogConflict.hidden = false;
    }
}

function hideConflictState() {
    if (teamDialogConflict) {
        teamDialogConflict.hidden = true;
    }
    teamDialogConflictRecord = null;
}

function showSaveAsState() {
    hideConflictState();
    teamDialogNicknameInput.focus();
    teamDialogNicknameInput.select();
}

function showActionState(action, record) {
    teamDialogActionState = { action, record };
    if (teamDialogActionTitle) {
        teamDialogActionTitle.textContent = action === 'load' ? 'Load selected team?' : 'Remove selected team?';
    }
    if (teamDialogActionSummary) {
        teamDialogActionSummary.textContent = action === 'load'
            ? `Loading "${record.nickname}" will replace the current roster.`
            : `Remove saved team "${record.nickname}"? This cannot be undone.`;
    }
    if (teamDialogActionConfirmButton) {
        teamDialogActionConfirmButton.textContent = action === 'load' ? 'Load' : 'Remove';
    }
    if (teamDialogAction) {
        teamDialogAction.hidden = false;
    }
}

function hideActionState() {
    if (teamDialogAction) {
        teamDialogAction.hidden = true;
    }
    teamDialogActionState = null;
}

function focusTeamDialogList() {
    if (teamDialogLoadList) {
        teamDialogLoadList.focus();
    }
}

async function confirmInlineAction() {
    const state = teamDialogActionState;
    if (!state) {
        hideActionState();
        return;
    }
    ...
    if (state.action === 'load') {
        const names = parseRosterText(state.record.roster);
        const skipped = loadTeamRoster(names, teamDialogTeam);
        if (skipped.length > 0) {
            alert(`Loaded ${getTeamLabelForSide(teamDialogTeam)} from "${state.record.nickname}", but skipped: ${skipped.join(', ')}.`);
        }
        hideActionState();
        await refreshTeamDialogRecords();
        closeTeamDialog();
    ...
    if (state.action === 'remove') {
        ...
        await new Promise((resolve, reject) => {
            const tx = db.transaction(teamStorageStoreName, 'readwrite');
            const store = tx.objectStore(teamStorageStoreName);
            const request = store.delete(state.record.nickname);
...
async function confirmConflictSave(action) {
    try {
        const conflictRecord = teamDialogConflictRecord;
        if (!conflictRecord) {
            hideConflictState();
            return;
        }
        ...
        if (action === 'merge') {
            await saveTeamRecord({
                nickname,
                team: teamDialogTeam,
                roster: combineUniqueRosters(currentRoster, parseRosterText(conflictRecord.roster)).join(',')
            }, true);
        ...
        console.error('Unable to resolve team save conflict', error);
        alert('Unable to save the team in this browser.');
    }
}
